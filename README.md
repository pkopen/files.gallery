修改过程：
 ...
 2.修改index.php。找到var CodeMirror = {};（在// load _files/js/custom.js前几行），在它下面打空行，插入以下代码（从代码中复制最佳，仅连续的一行），再打空行：
...
files.gallery存储库中此单文件已编辑好。 放到网站目录，用浏览器首次访问，将自动创建配置文件_files/config/config.php。访问网页，编辑'username' => 'pkopen','password' => 'md5值'；编辑'allow_all' => true。
